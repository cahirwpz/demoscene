...
from common import Bit, Word, Array, Blit, Blit1, Channel


def c2p_2_1_4_blitter_mangled(fast=False, bitplane_output=True):
  m0 = Word.Mask('00ff')
  m1 = Word.Mask('0f0f')
  m2 = Word.Mask('5555')
  ...
  if fast:
    ...
    # following operation can be done with bitplane shifter and extra mask
    # bitplane
    Blit1(lambda a: ((a >> 1) & m2),
          N / 4, 1, Channel(C, 0, 1), Channel(D[0], 0, 0))
    Blit1(lambda a: ((a >> 1) & m2),
          N / 4, 1, Channel(C, 1, 1), Channel(D[2], 0, 0))
    Blit1(lambda a: (a & m2),
          N / 4, 1, Channel(C, 0, 1), Channel(D[1], 0, 0))
    Blit1(lambda a: (a & m2),
          N / 4, 1, Channel(C, 1, 1), Channel(D[3], 0, 0))
    print("Bitplanes:")
    Array.Print("[0]:", *D[0])
    Array.Print("[1]:", *D[1])
    Array.Print("[2]:", *D[2])
    Array.Print("[3]:", *D[3])
  elif bitplane_output:
