...
with open(argv[1], 'r') as f:
    ...
    # read header
    header_pattern = re.compile(r'x\s*=\s*(\d+)\s*,\s*y\s*=\s*(\d+)(?:\s*,\s*rule\s*=\s*(\w+\/\w+)\s*)*')
    ...
    im = Image.new("P", (x, y)) # pallete mode (only this can produce 1-bit image, 1 and L seem to always output 8-bit)
    cur_x = 0
    cur_y = 0
    for line in f:
        captures = [(int(count) if count != '' else 1, tag) for count, tag in rle_pattern.findall(line)]
