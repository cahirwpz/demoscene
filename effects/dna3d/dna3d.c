...
static short dna_radius = 500;
static short dna_angle_offset = 300;
static short dna_angle_start = 0;
static short dna_z_offset = 160;

static void ManipulateDnaParams(void) {
    dna_angle_offset = normfx(SIN(frameCount * 15) * 300);
    dna_z_offset = 140 + normfx(SIN(frameCount * 10) * 20);
    dna_radius = 600 + normfx(SIN(frameCount * 12) * 300);
}

static void Init(void) {
  GenerateDnaPoints(mesh, dna_radius, dna_angle_offset, dna_angle_start, dna_z_offset);  
...
static void DrawObject(Object3D *object, BitmapT *dst) {
  ...
  do {
    ...
    if (np == 2) {
      /* Why BlitterLine(xp, yp, x, y) doesn't work? */
      short disty = abs(y - yp);
      if (yp > y) {
        BlitterLineSetup(dst, 1, LINE_EOR | LINE_ONEDOT);
        BlitterLine(xp, 385 + disty, x, 385 - disty);
      } else {
        BlitterLineSetup(dst, 1, LINE_EOR | LINE_ONEDOT);
        BlitterLine(xp, 385 - disty, x, 385 + disty);
      } 
