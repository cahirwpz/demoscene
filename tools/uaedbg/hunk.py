...
from .stabs import StringTable, Stab
...
class HunkSymbol(Hunk):
    ...
    def dump(self):
        ...
        width = max(len(s.name) for s in self.symbols) + 1

        for s in sorted(self.symbols, key=lambda s: s.name):
            print('  {0}: {1}'.format(s.name.ljust(width, ' '), s.refs))
    ...
    def dump(self):
        ...
        for u in self.units:
            ...
            for h in u[2]:
                ...
                if h[4]:
                    print('     ', 'DEFS')
                    width = max(len(s[0]) for s in h[4])
                    for s in sorted(h[4], key=lambda x: x[1]):
                        print('       ', s[0].ljust(width), '=', s[1])
            print('')
